...
from quantization.quant import FakeQuantizer
...
class QLinear(nn.Linear):
    def __init__(
        ...
        self.bit_config = self.q_config.bit_config # {input:-, weight:-, output:-, bias:-}

        self.input_quantizer = FakeQuantizer.build(self.q_type.input, **self.bit_config.input)
        self.weight_quantizer = FakeQuantizer.build(self.q_type.weight, **self.bit_config.weight)
        self.bias_quantizer = FakeQuantizer.build(self.q_type.bias, **self.bit_config.bias)
