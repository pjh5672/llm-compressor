...
@torch.no_grad()
def ria(
    model, device, sparsity_ratio, alpha, n_samples=512, seq_len=2048, verbose=True
):
    if verbose:
        LOGGER.info("Pruning model... [Prune-method : RIA]")
    ...
    for i in pg_bar:
        ...
        for name in subset:
            ...
            ) * (torch.sqrt(subset[name].scaler_row.reshape((1, -1)))) ** alpha
