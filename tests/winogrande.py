...
def build_prompt(example):
    ...
    sentence = example["sentence"][:pronoun_loc].strip()
    context1 = example["option1"] + example["sentence"][pronoun_loc + 1:]
    context2 = example["option2"] + example["sentence"][pronoun_loc + 1:]
    return sentence, (context1, context2)
...
def compute_winogrande(model, tokenizer, val_df):
    cors = []
    device = model.device

    for i in range(val_df.shape[0]):
        example = val_df.iloc[i]
        sentence, ctx = build_prompt(example)

        scores = {}
        scores["1"] = log_likelihood(sentence, f" {ctx[0]}")
        scores["2"] = log_likelihood(sentence, f" {ctx[1]}")
...
def main(model, tokenizer, n_samples=None):
    ...
    cors = compute_winogrande(model, tokenizer, val_df)
